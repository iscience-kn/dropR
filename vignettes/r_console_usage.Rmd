...
dropRdemo$do_idx <- extract_drop_out_from_df(dropRdemo,q_pos = qs)
```
...
```{r}
library(kableExtra)
stats_table <- compute_stats(dropRdemo, by_cond = "experimental_condition",
              do_indicator = "do_idx",
              no_of_vars = length(qs))
# kable(stats_table)
```

Out of `r nrow(dropRDemo)` participants in the demo sample, `r stats_table$remain[1]` remain in the survey, accounting for `r round(stats_table$pct_remain[1]* 100,digits=2)` percent of the sample.
